...
// Array of all files needed to run offline
...
self.addEventListener('install', e => {
  e.waitUntil(
    caches.open(APP_VERSION).then(cache => {
      // Use addAll() to cache both the HTML and version.js
      return cache.addAll(FILES_TO_CACHE).catch(err => console.error('Failed to cache app:', err));
...
self.addEventListener('fetch', e => {
  ...
  // Only apply offline app logic if the URL actually requests the HTML file
  if (e.request.mode === 'navigate' && reqUrl.pathname.endsWith('MatNela.html')) {
    e.respondWith(
  ...
  // Default handler for everything else (like version.js, or visiting the README root)
  e.respondWith(
