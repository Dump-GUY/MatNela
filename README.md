# 🍎 MatNela — Matematika s Nelou / Math with Nela

### 🔗 App URL
> [https://dump-guy.github.io/MatNela/MatNela.html](https://dump-guy.github.io/MatNela/MatNela.html)
---

> CZ [Česky](#cz-česky) &nbsp;|&nbsp; EN [English](#en-english)

---

## CZ Česky

### O aplikaci

**MatNela** je interaktivní matematická aplikace pro děti ve věku **6–12 let**. Procvičuje sčítání, odčítání, násobení a dělení formou hry s vizualizací jablek. Navržena s cyberpunk designem, který děti baví.

**Klíčové vlastnosti:**
- ➕ **Sčítání**
- ➖ **Odčítání**
- ✖️ **Násobení**
- ➗ **Dělení**
- ⭐ **Vše dohromady** — smíšený režim
- 🍎 Vizualizace výsledků pomocí jablek a košíků
- 🌓 Tmavý i světlý cyberpunk režim
- 🌐 Čeština a angličtina (automatická detekce jazyka systému)
- 🔊 Zvukové efekty pro správnou i špatnou odpověď
- ⚙️ Nastavení obtížnosti — rozsahy čísel pro každou operaci zvlášť
- 🏆 Výsledky s hvězdičkami po každé sadě 10 příkladů
- 📱 Funguje jako PWA — instalovatelná aplikace bez app storu
- ✈️ **Plně funkční offline** po prvním načtení

---

### 🚀 Spuštění bez internetu — lokálně ze souboru

Aplikaci lze spustit **přímo z počítače nebo telefonu bez internetu a bez instalace**:

1. Stáhni soubor **`MatNela.html`**
2. Otevři ho v libovolném prohlížeči (Chrome, Firefox, Safari, Edge)
3. Hotovo — aplikace funguje okamžitě, nic dalšího není potřeba

> 💡 Soubor lze sdílet přes USB, e-mail, WhatsApp nebo Bluetooth. Funguje na Windows, macOS, Linux, Android i iOS.

---

### 📱 Instalace jako PWA — Android (Google Chrome)

PWA (Progressive Web App) nainstaluje aplikaci na plochu jako normální app — bez app storu, bez poplatků.

**Podmínka:** Aplikace musí být dostupná přes HTTPS adresu (ne lokální soubor).

1. Otevři URL aplikace v **Google Chrome**
2. Chrome automaticky nabídne banner *„Přidat na plochu"* — klepni na něj
3. Nebo klepni na **⋮** (tři tečky vpravo nahoře) → **Přidat na plochu** → **Přidat**
4. Ikona 🍎 se objeví na ploše — aplikace se otevírá jako samostatná app bez lišty prohlížeče

> ✅ Po instalaci funguje **plně offline**. Aplikace se zakešuje při prvním načtení.

---

### 🍎 Instalace jako PWA — iPhone / iPad (Safari)

**Podmínka:** Aplikace musí být dostupná přes HTTPS adresu (ne lokální soubor).

1. Otevři URL aplikace v **Safari** *(Chrome na iOS PWA nepodporuje)*
2. Klepni na tlačítko **Sdílet** — ikona čtverce se šipkou nahoru ↑ (uprostřed dolní lišty)
3. Přejdi dolů a vyber **Přidat na plochu**
4. Uprav název (nebo ponech `MatNela`) → klepni **Přidat**
5. Ikona 🍎 se objeví na ploše

> ✅ Po instalaci funguje **plně offline**.

---

### 💻 Instalace jako PWA — Windows (Google Chrome)

**Podmínka:** Aplikace musí být dostupná přes HTTPS adresu (ne lokální soubor).

1. Otevři URL aplikace v **Google Chrome** na počítači
2. V adresním řádku vpravo se zobrazí ikona **⊕** (Instalovat aplikaci)
3. Klikni na ni → **Instalovat**
4. Aplikace se otevře ve vlastním okně bez prohlížeče a přidá se do nabídky Start / Launchpadu

> ✅ Po instalaci funguje **plně offline**.

---

### 🔒 Soukromí a bezpečnost

- Aplikace **neodesílá žádná data** nikam na internet
- Veškerá nastavení se ukládají **pouze lokálně** v prohlížeči (localStorage)
- Žádný tracking, žádná analytika, žádné cookies třetích stran
- Zdrojový kód je plně otevřený a čitelný v jednom HTML souboru

---

### 📄 Licence

**Volné použití pro vzdělávací a nekomerční účely.**

```
Copyright (c) MatNela Contributors

Povoleno:
✅ Používat osobně, ve škole nebo ve školce
✅ Sdílet s rodiči, učiteli a dětmi
✅ Upravovat zdrojový kód pro vlastní potřeby

Zakázáno:
❌ Jakékoliv zpoplatnění aplikace nebo jejích úprav
❌ Hostování nebo šíření upravené verze bez uvedení odkazu
   na původní zdrojový repozitář
```

Při jakémkoliv hostování nebo sdílení upravené verze je **povinné** uvést viditelný odkaz na původní repozitář.

> Vytvořeno zdarma pro podporu vzdělávání dětí. ❤️

---

## EN English

### About the App

**MatNela** is an interactive math app for children aged **6–12**. It practices addition, subtraction, multiplication and division through a game with apple visualizations. Designed with a cyberpunk aesthetic that kids love.

**Key features:**
- ➕ **Addition**
- ➖ **Subtraction**
- ✖️ **Multiplication**
- ➗ **Division**
- ⭐ **All together** — mixed mode
- 🍎 Visual representation of results using apples and baskets
- 🌓 Dark and light cyberpunk theme
- 🌐 Czech and English (auto-detects system language)
- 🔊 Sound effects for correct and wrong answers
- ⚙️ Difficulty settings — number ranges for each operation individually
- 🏆 Results with stars after every set of 10 problems
- 📱 Works as a PWA — installable app without an app store
- ✈️ **Fully offline capable** after first load

---

### 🚀 Quick Start — Offline use (local file)

The app can be run **directly from your computer or phone without internet and without installation**:

1. Download the file **`MatNela.html`**
2. Open it in any browser (Chrome, Firefox, Safari, Edge)
3. That's it — the app works immediately, nothing else needed

> 💡 The file can be shared via USB, email, WhatsApp or Bluetooth. Works on Windows, macOS, Linux, Android and iOS.

---

### 📱 PWA Installation — Android (Google Chrome)

A PWA (Progressive Web App) installs the app on your home screen like a regular app — no app store, no fees.

**Requirement:** The app must be available via an HTTPS URL (not a local file).

1. Open the app URL in **Google Chrome**
2. Chrome will automatically show a banner *"Add to Home Screen"* — tap it
3. Or tap **⋮** (three dots top-right) → **Add to Home Screen** → **Add**
4. The 🍎 icon appears on your home screen — the app opens standalone without the browser bar

> ✅ After installation it works **fully offline**. The app is cached on first load.

---

### 🍎 PWA Installation — iPhone / iPad (Safari)

**Requirement:** The app must be available via an HTTPS URL (not a local file).

1. Open the app URL in **Safari** *(Chrome on iOS does not support PWA)*
2. Tap the **Share** button — the square with arrow icon ↑ (bottom center bar)
3. Scroll down and select **Add to Home Screen**
4. Edit the name (or keep `MatNela`) → tap **Add**
5. The 🍎 icon appears on your home screen

> ✅ After installation it works **fully offline**.

---

### 💻 PWA Installation — Windows (Google Chrome)

**Requirement:** The app must be available via an HTTPS URL (not a local file).

1. Open the app URL in **Google Chrome** on your computer
2. An install icon **⊕** appears on the right side of the address bar
3. Click it → **Install**
4. The app opens in its own window without the browser and is added to Start Menu / Launchpad

> ✅ After installation it works **fully offline**.

---

### 🔒 Privacy & Security

- The app **sends no data** anywhere on the internet
- All settings are saved **locally only** in the browser (localStorage)
- No tracking, no analytics, no third-party cookies
- The source code is fully open and readable in a single HTML file

---

### 📄 License

**Free to use for educational and non-commercial purposes.**

```
Copyright (c) MatNela Contributors

Permitted:
✅ Personal use, at school or kindergarten
✅ Sharing with parents, teachers and children
✅ Modifying the source code for personal use

Prohibited:
❌ Any form of charging money for the app or its modifications
❌ Hosting or distributing modified versions without crediting
   the original source repository with a visible link
```

When hosting or sharing any modified version, a **visible link** to the original source repository is required.

> Created free of charge to support children's education. ❤️

---

*Made with ❤️ for kids learning math*
